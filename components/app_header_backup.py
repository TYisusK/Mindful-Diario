...
def AppHeader(page: ft.Page, active_route: str):
    ...
    - Mobile: shows a bottom nav bar with main actions; hamburger menu
      contains quick links (Inicio, Ayuda profesional, Estadísticas, Cerrar sesión).
    ...
    def show_menu():
        ...
        chips = ft.Column(
            [
                ft.Container(
                    on_click=(lambda e, r=p: (logout(e) if r == "logout" else go_to(r))),
    ...
    def make_header():
        ...
        if not is_small_screen():
            right = ft.Row([build_nav_row(), _push_button()], alignment=ft.MainAxisAlignment.END, spacing=8)
        else:
            right = ft.Row([_push_button()], alignment=ft.MainAxisAlignment.END)
